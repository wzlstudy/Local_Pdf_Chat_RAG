...
### 启动API服务
```bash
pip install fastapi uvicorn python-multipart  # 新增依赖
...
curl -X POST "http://localhost:17995/api/ask" \
  ...
  -d '{"question": "文档的核心观点是什么？", "enable_web": true}'
```

#### 3. 直接调用Ollama（兼容OpenAI格式）
```python
...
| 参数     | 类型    | 必填 | 说明                  |
|----------|---------|------|---------------------|
| question | string  | 是   | 问题内容（1-500字）   |
| enable_web| boolean | 否   | 是否启用网络搜索增强  |
